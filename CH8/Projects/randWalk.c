...
int main(void) {
    ...
    while(step <= 'Z') {
        ...
        while (!can_move) {
            ...
            switch (direction) {
                case UP:
                    if (coord_y - 1 >= 0 && map[coord_x][coord_y - 1] == '.') {
                        coord_y--;
                        can_move = true;
                    } 
                    break;
                case RIGHT:
                    if (coord_x + 1 <= 9 && map[coord_x + 1][coord_y] == '.') {
                        coord_x++;
                        can_move = true;
                    } 
                    break;
                case DOWN:
                    if (coord_y + 1 <= 9 && map[coord_x][coord_y + 1] == '.') {
                        coord_y++;
                        can_move = true;
                    } 
                    break;
                case LEFT:
                    if (coord_x - 1 >= 0 && map[coord_x - 1][coord_y] == '.') {
                        coord_x--;
                        can_move = true;
                    } 
                    break;
